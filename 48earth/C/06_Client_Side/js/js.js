...
$(function(){
    ...
    // click start image button
    $("#start").click(function(){
        canvas.start_stop = true;
        $("#start").hide()
        $("#stop").show();
        $("#audio1").get(0).play();
        $("#power_color").addClass('oil');
        canvas.game_start();
    })

    // click stop image button
    $("#stop").click(function(){
        canvas.start_stop = false;
        $("#start").show()
        $("#stop").hide();
        $("#audio1").get(0).pause();
        $("#power_color").removeClass('oil');
        canvas.game_stop();
    })

    // click sound_on image button
...
function Canvas(){
    ...
    this.bg = 0;
    this.bg_img = [];
    this.bg_img_width = [0, 50, 80, 120, 150, 180, 210];
}
...
Shape.prototype.draw = function(){//更新角色位置
    switch(this.name){
        case"shape0":
        ...
        break;
        case"shape1":
            this.left = this.left - 2.8 - (canvas.label/3);    
        break;
        case"shape2":
            this.left = this.left - 2.6 - (canvas.label/3);
        break;
        case"shape3":
            this.left = this.left - 2.4 - (canvas.label/3);
        break;
        case"attack1":
            this.left = this.left + 3.5 + (canvas.label);
        break;
        case"attack2":
            this.left = this.left - 3.5 - (canvas.label);
        break;
...
Shape.prototype.shape_add = function(type){//新增角色
    $("#canvas").append('<img src="06_design/06_source/shape'+type+'1.png" class="shape'+type+' img center">');
...
Shape.prototype.attack_add = function(obj, type){//新增攻擊
    $("#canvas").append('<img src="06_design/06_source/attack'+type+'.png" class="attack'+type+' img center">');
...
Shape.prototype.supply_add = function(){//新增油桶
    $("#canvas").append('<img src="06_design/06_source/supply.png" class="supply img center">');
...
Shape.prototype.effect = function(){//更新特效
    if(this.name == "shape0"){
        this.number += 1;
        if(this.number > 2){
            this.number = 1;
        }
        $(this.obj).attr('src', '06_design/06_source/'+this.name+this.number+'.png');
...
Canvas.prototype.shape_add = function(){//新增角色
    var time = Math.abs(180 - (this.label*5));
    var time = Math.abs(180 - (this.label*5));
    if(time <= 90){
        time = 90;
    }
...
Canvas.prototype.attack_add = function(type){//新增攻擊
    if(type == 1){
        ...
        var time = Math.abs(120 - (this.label*5));
        if(time <= 10){
            time = 10;
        }
...
Canvas.prototype.supply_add = function(type){//新增油桶
    var time = Math.abs(300 - (this.label*5));
    if(time <= 120){
        time = 120;
    }
...
Canvas.prototype.effect = function(){//更新特效
    this.effect_time += 2;
    ...
    for(i = 1; i <= 6 ; i++){
        this.bg_img[i] -= (i*2);
        if(this.bg_img[i] < (0-this.bg_img_width[i])){
            this.bg_img[i] = 960;
        }
        $("#bg_img"+i).css('left',  this.bg_img[i]);
    }
}
        ...
        for(i = 0 ; i < this.shape3.length ; i++){
            if(
                ...
                if(this.shape3[i].power == 0){
                    ...
                    $(this.shape3[i].obj).attr('src', '06_design/06_source/shape32.png');
                }
                return false;
            } 
        }
...
Canvas.prototype.ready_remove = function(){//刪除延遲
    for(i = 0 ; i < this.shape1.length ; i ++){
...
Canvas.prototype.controller_change = function(){//更新遊戲控制區
    ...
    $("#game").css('background-position-x', this.bg+"px");
}
Canvas.prototype.water = function(obj){//水波特效
    $(obj).append('<div class="water" style="width: '+$(obj).width()+'px; height: '+$(obj).width()+'px;"></div>');
    window.setTimeout(function(){
        $(".water").remove();
    }, 400)
}
Canvas.prototype.over = function(){//搖晃特效
    $("#game").addClass('over');
    window.setTimeout(function(){
        $("#game").removeClass('over');
    }, 400)
}
Canvas.prototype.bg_img_change = function(){//背景星球位置
    for(i = 1; i <= 6 ; i++){
        var top = this.random(30*i, 30*i+30);
