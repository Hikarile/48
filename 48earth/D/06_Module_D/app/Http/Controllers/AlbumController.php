...
class AlbumController extends Controller
{
    ...
    public function add(Request $request){
        ...
        return response()->view('album.add', $album, 200)->header('Content-Type', 'application/xml');
    }
    ...
    public function delete(Request $request, $album_id = ''){
        ...
        $album->delete();
        return response()->view('album.delete', compact('image'), 200)->header('Content-Type', 'application/xml');
    }
        ...
        switch(count($data)){
            ...
            case"2":
                for($i = 0; $i <=1; $i++){
                    ...
                    imagecopyresized($new_image, $or_image, $i*45, 0, ($data[$i]['width']/2) - ($w / 2), ($data[$i]['height']/2) - ($h / 2), 45 ,90, $w, $h);
                }
            break;
            case"3":
                $or_image = imagecreatefromjpeg($data[0]['name']);
                ...
                for($i = 1; $i <=2; $i++){
                    ...
                    imagecopyresized($new_image, $or_image, ($i-1)*45, 45, ($data[$i]['width']/2) - ($w / 2), ($data[$i]['height']/2) - ($h / 2), 45 ,45, $w, $h);
                }
            break;
        }
