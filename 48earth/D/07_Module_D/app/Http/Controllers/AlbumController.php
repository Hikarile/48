...
class AlbumController extends Controller
{
    ...
    public function latest(Request $request, $album_id){
        $album = Album::where('album_id', $album_id)->firstorFail();
        $account = Account::where('id', $album->account_id)->firstorFail();
        return response()->view('album.latest', compact('album', 'account'), 200)->header('Content-Type', 'application/xml');
    }
    
    public function hot(Request $request, $album_id){
        $album = Album::where('album_id', $album_id)->firstorFail();
        $account = Account::where('id', $album->account_id)->firstorFail();
        return response()->view('album.hot', compact('album', 'account'), 200)->header('Content-Type', 'application/xml');
    }
    ...
    public function delete(Request $request, $album_id){
        $album = $request->account->albums()->where('album_id', $album_id)->first();
        if($album == null){
            return abort('401');
        }
    ...
    public function move(Request $request){
        ...
        $image = Image::where('delete_token', "")->where('image_id', $data['src_image'])->firstOrFail();
    ...
    public function cover(Request $request, $album_id){
        ...
        foreach($covers as $key => $cover){
            $image = Image::where('image_id', $cover)->where('delete_token' , '')->first();
            $tf = true;
            if($key > 0){
                foreach($data as $val){
        ...
        switch(count($data)){
            ...
            for($i = 0; $i <= 1;$i++){
                $or_image = imagecreatefromjpeg($data[0]['name']);
                if($data[0]['width']*2 < $data[0]['height']){
                    $w = $data[0]['width']*2;
                    $h = $data[0]['width'];
                }else{
                    $w = $data[0]['height']/2;
                ...
                imagecopyresized($new_image, $or_image, 45*$i, 0, ($data[0]['width']/2)-($w/2), ($data[0]['height']/2)-($h/2), 45, 90, $w, $h);
            }
            break;
            case"3":
            $or_image = imagecreatefromjpeg($data[0]['name']);
            ...
            for($i = 1; $i <= 2;$i++){
                ...
                imagecopyresized($new_image, $or_image, 45*($i-1), 45, ($data[0]['width']/2)-($w/2), ($data[0]['height']/2)-($h/2), 45, 45, $w, $h);
            }
            break;
        }
